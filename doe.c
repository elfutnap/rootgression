...
#define MAX 256
Float_t t=3.182;			//T of student for 3 replicas 95% of confidence
Int_t lineBuffer=999;		//Line buffer
Int_t error=0;			//Number of errors overall

void doe(const Char_t *inputfile){

	/*======= Variables declaration =======*/
	printf("\n  1 - Opening input file '%s'.\n",inputfile); //Check file
	...
	Float_t spe, sb, sb_crit;	//Errore puramente sperimentale corretto per t value
	...
	printf("	Insert a valid pure experimental error:	");	//spe error 
	scanf("%f",&spe);
	...
	if (ifp == NULL) {			//Warning can't open file
		fprintf(stderr, "\n  ERROR: Can't open input file!\n");
  		error=1;
        	...
        	i++;
		printf("  3 - Input file opened successfully.\n"); //File opened
	}
	...
	for(i=0; i<experiments;i++){
		SS_Mean = SS_Mean+arr_answers[i];
	}
	SS_Mean = pow(SS_Mean/experiments,2);
	printf("Sum of squares due to the mean: %lf\nFreedom degrees: 1\n",SS_Mean); //Print sum of squares due to the mean, freedom degrees
	for(i=0; i<experiments;i++){
		SS_Corr = SS_Corr+ pow(arr_answers[i]-SS_Mean,2);
	}
	printf("Sum of squares corrected for the mean: %lf\nFreedom degrees: %d\n",SS_Corr, experiments-1); //Print sum of squares corrected for the mean, freedom degrees
	for(i=0; i<experiments;i++){
		SS_Fact = SS_Fact+ pow(predicted(i,0)-SS_Mean,2);
	}
	printf("Factor sum of squares: %lf\nFreedom degrees: %d\n",SS_Fact, matrix_columns-1); //Print factor sum of squares, freedom degrees
	for(i=0; i<experiments;i++){
		SS_Res = SS_Res+ pow(residuals(i,0),2);
	}
	printf("Residual sum of squares: %lf\nFreedom degrees: %d\n",SS_Res, experiments-factors); //Print sum of squares corrected for the mean, freedom degrees
	printf("t of student:	%f\nexperimental error:	%f\nUncertainty of the esteem of the effects:	%f\nCritical uncertainty:	%f\n", t, spe, sb, sb_crit);
	printf("\n\nR^2: %lf\n",SS_Fact/SS_Corr); //Correlation coefficient of the model
	//printf("Constant of Fischer: %lf\n",); // Fischer constants
	printf("Fischer's F: %lf\n\n",SS_Fact/SS_Res); // Fischer test of the model
	//printf("Fischer's F: %lf\n",); // Significativity
